# -*- coding: utf-8 -*-
    ...
    # Strand bias feature removed to reduce dependency on strand information
    ...
    # Feature combination (strand bias removed)
    ...
    for name, classifier in classifiers.items():
        if name == 'RandomForest':
            ...
            # Convert data to DMatrix format for XGBoost GPU acceleration
            dtrain = DMatrix(X_train_split, label=y_train_split, device_id=0)
            dval = DMatrix(X_val, label=y_val, device_id=0)
            param_grid = {'max_depth': [3, 5, 7], 'learning_rate': [0.01, 0.1, 0.3], 'n_estimators': [100, 200, 300]}
            grid_search = GridSearchCV(classifier, param_grid, cv=5, scoring='f1_weighted', n_jobs=-1)
            grid_search.fit(X_train_split, y_train_split, eval_set=[(X_val, y_val)], early_stopping_rounds=10)
            y_val_pred = grid_search.predict(X_val)
    ...
    if isinstance(best_classifier, XGBClassifier):
        dtrain_full = DMatrix(X_train_combined_selected, label=y_train, device_id=0)
        dtest = DMatrix(X_test_combined_selected, device_id=0)
        best_classifier.fit(dtrain_full)
        y_test_pred = best_classifier.predict(dtest) + 1
    else:
